...
export async function getAdminResolutionTimeTrend(
  ...
  for (const { submitted_at, resolved_at } of ticketResolutions.values()) {
  ...
  for (const { totalHours: h, count: c } of periodData.values()) {
    totalHours += h;
    totalTickets += c;
  }
...
export async function getAdminCreatedVsResolved(
  ...
  for (const changedAt of resolvedTickets.values()) {
