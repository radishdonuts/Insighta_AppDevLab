...
def _find_insighta_root(download_dir: Path) -> Path:
    ...
    raise FileNotFoundError(
        f"Could not locate Insighta root under {download_dir}. Expected folders: artifacts/ and data/nlp/."
    )
...
def main() -> int:
    ...
    parser.add_argument("--repo-root", default=str(repo_root_default), help="Repo root path")
    parser.add_argument("--download-dir", default=".cache/drive_sync", help="Temp download directory under repo root")
    parser.add_argument(
        "--artifact-cache-dir",
        default="artifacts-cache",
        help="Local artifact cache dir under repo root",
    )
    args = parser.parse_args()

    repo_root = Path(args.repo_root).resolve()
    download_dir = (repo_root / args.download_dir).resolve()
    artifact_cache_dir = (repo_root / args.artifact_cache_dir).resolve()

    print(f"[sync] repo_root={repo_root}")
    print(f"[sync] download_dir={download_dir}")
    print(f"[sync] artifact_cache_dir={artifact_cache_dir}")
